...
describe QuestionGroup, type: :model do
  ...
  context "when creating" do
    ...
    it "reports DOM ready #css_class based on dependencies" do
      ...
      expect(question_group.css_class(response_set)).to eq("g_default g_dependent")

      expect(dependency).to receive(:is_met?).and_return(false)
      expect(question_group.css_class(response_set)).to eq("g_default g_dependent g_hidden")

      question_group.custom_class = "foo bar"
      expect(dependency).to receive(:is_met?).and_return(false)
      expect(question_group.css_class(response_set)).to eq("g_default g_dependent g_hidden foo bar")
    end
  end
