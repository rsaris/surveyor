...
module Surveyor
  module SurveyorControllerMethods
    ...
    def create
      surveys = Survey.where(:access_code => params.require(:survey_code)).order("survey_version DESC")
      if !params.has_key?(:survey_version)
        @survey = surveys.first
      else
        permitted = params.permit(:survey_version)
        @survey = surveys.where(:survey_version => permitted[:survey_version]).first
      end
    ...
    def update
      permitted_params = params.permit(:r, :finish, :section, :response_set_code)

      question_ids_for_dependencies = (permitted_params[:r] || []).map{|k,v| v["question_id"] }.compact.uniq
      saved = load_and_update_response_set_with_retries

      return redirect_with_message(surveyor_finish, :success, t('surveyor.completed_survey')) if saved && permitted_params[:finish]
      ...
      respond_to do |format|
        format.html do
          if @response_set.nil?
            ...
            redirect_to surveyor.edit_my_survey_path(:anchor => anchor_from(permitted_params[:section]), :section => section_id_from(params))
          end
        end
        format.js do
          if @response_set
            ...
            render :text => "No response set #{permitted_params[:response_set_code]}",
              :status => 404
          end
        end
      end
    ...
    def load_and_update_response_set
      ResponseSet.transaction do
        permitted = params.permit(:response_set_code, :r, :finish)

        @response_set = ResponseSet.includes({:responses => :answer}).where(:access_code => permitted[:response_set_code]).first
        if @response_set
          saved = true
          if permitted[:r]
            @response_set.update_from_ui_hash(permitted[:r])
          end
          if permitted[:finish]
    ...
    def export
      permitted = params.permit(:survey_code, :survey_version)

      surveys = Survey.where(:access_code => permitted[:survey_code]).order("survey_version DESC")
      s = permitted[:survey_version].blank? ? surveys.first : surveys.where(:survey_version => permitted[:survey_version]).first
    ...
    def set_response_set_and_render_context
      permitted = params.permit(:response_set_code)

      @response_set = ResponseSet.includes({:responses => [:question, :answer]}).where(:access_code => permitted[:response_set_code]).first
      @render_context = render_context
    end

     def set_locale
       permitted = params.permit(:new_locale, :locale)

      if permitted[:new_locale]
        I18n.locale = permitted[:new_locale]
      elsif permitted[:locale]
        I18n.locale = permitted[:locale]
      else
    ...
    def section_id_from(parameters)
      permitted = parameters.permit(:section, :current_section)

      if permitted[:section] && permitted[:section].respond_to?(:keys)
        permitted[:section].keys.first
      elsif permitted[:section]
        permitted[:section]
      elsif permitted[:current_section]
        permitted[:current_section]
      end
    end

    def anchor_from(section_param)
      if section_param.respond_to?(:keys) &&
          section_param[section_param.keys.first].respond_to?(:keys)
        section_param[section_param.keys.first].keys.first
      else
        nil
      end
    end
    ...
    def determine_if_javascript_is_enabled
      permitted = params.permit(:surveyor_javascript_enabled)

      if permitted[:surveyor_javascript_enabled] && permitted[:surveyor_javascript_enabled].to_s == "true"
