  ...
  module Models
    module ResponseSetMethods
      ...
      included do
        ...
        belongs_to :current_section, :foreign_key => :current_section_id, :class_name => :survey_section, optional: true
      ...
      def is_unanswered?(question)
        self.responses.detect{|r| r.question_id == question.id && !r.to_formatted_s.blank?}.nil?
      end
      ...
      def all_dependencies(question_ids = nil)
        ...
        {
          :show => arr[0].map{|d| d.id_for_dom},
          :hide => arr[1].map{|d| d.id_for_dom}
        }
      end
