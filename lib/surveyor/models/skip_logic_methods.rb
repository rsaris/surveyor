  ...
  module Models
    module SkipLogicMethods
      ...
      def is_met?(response_set)
        ...
        rgx = Regexp.new(self.skip_logic_conditions.map{|slc| ["a","o"].include?(slc.rule_key) ? "\\b#{slc.rule_key}(?!nd|r)\\b" : "\\b#{slc.rule_key}\\b"}.join("|")) # exclude and, or
      ...
      def conditions_hash(response_set)
        hash = {}
        self.skip_logic_conditions.each{|slc| hash.merge!(slc.to_hash(response_set))}
        return hash
      end
    end
  end
